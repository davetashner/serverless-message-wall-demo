...
echo ""
echo "Next steps:"
echo "  1. Create ConfigHub worker with org-admin access:"
echo "     cub worker create --space order-platform-ops-dev argocd-reader --org-role admin"
echo "  2. Create credentials secret:"
echo "     kubectl create secret generic confighub-actuator-credentials \\"
echo "       --namespace argocd --context ${CLUSTER_CONTEXT} \\"
echo "       --from-literal=CONFIGHUB_WORKER_ID=<WORKER_UUID> \\"
echo "       --from-literal=CONFIGHUB_WORKER_SECRET=<WORKER_SECRET>"
echo "  3. Apply ApplicationSet for Order Platform:"
